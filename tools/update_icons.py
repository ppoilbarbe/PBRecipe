#!/usr/bin/env python3
"""Sync icons in src/pbrecipe/resources/icons/ from the PBIcons project.

Two kinds of files live in that directory, both sourced from PBIcons:

- The app icon itself: `pbrecipe.ico`, `pbrecipe.icns`, `pbrecipe-128x128.png`
  and `pbrecipe-512x512.png` (`_APP_ICON_FILES` below). These used to be
  generated locally from a source PNG via the now-retired
  `tools/make_icons.py`; they are copied ready-made from PBIcons instead.
- Toolbar/action SVG icons (`delete.svg`, `save.svg`, …): every `*.svg`
...
Usage:
    ...
    python tools/update_icons.py delete.svg     # sync only the named file(s)
...
RESOURCES_DIR = ROOT / "src" / "pbrecipe" / "resources" / "icons"

# App icon files, always synced regardless of what's already on disk.
_APP_ICON_FILES = (
    "pbrecipe-128x128.png",
    "pbrecipe-512x512.png",
    "pbrecipe.ico",
    "pbrecipe.icns",
)

# PBIcons subdirectories to search for an icon, in priority order.
_ICON_DIRS = ("actions", "programs")
...
_USER_AGENT = "pbrecipe-update-icons"
