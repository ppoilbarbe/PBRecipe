...
.DEFAULT_GOAL := help
.PHONY: help update-icons venv venv-update install run test test-php coverage lint format \
        dist srcdist clean hooks update-vendors _php-vendor \
        bump-release bump-year bump-set \
        docs docs-live live-test

update-icons: ## Sync app icons (.ico/.icns/.png) and toolbar SVGs from PBIcons
